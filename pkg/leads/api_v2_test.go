...
func TestGetLeontelValuesNew(t *testing.T) {
	assert := assert.New(t)
...
func TestLeadToLeontelNew(t *testing.T) {
	assert := assert.New(t)
...
func TestInsertLead(t *testing.T) {
	...
	for _, test := range tests {
		...
		mock.ExpectPrepare("INSERT INTO leads").
			ExpectExec().
			WithArgs(test.Lead.SouID, test.Lead.LeatypeID, test.Lead.LeaURL, test.Lead.LeaPhone).
			WillReturnResult(sqlmock.NewResult(1, 1))

		err := ch.Storer.Insert(test.Lead)
		// err := d.Insert(test.Lead)
		assert.NoError(err)
...
// func TestSendLeadToLeontelNew(t *testing.T) {
// 	assert := assert.New(t)
